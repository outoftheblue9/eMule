...
void CPartFile::ResumeFile(bool resort)
{
	...
	if (status == PS_ERROR && m_bCompletionError) {
		...
		// UpdateDisplayedInfo) when there's nothing to resume â€” the file
...
void CPartFile::FlushBuffer(bool bForceICH, bool bNoAICH)
{
	...
	try {
		...
		// if the file is already paused-insufficient â€” the handler has already
	...
	try {
		...
		if (newsize > cursize) {
			...
			if (pAlloc && pAlloc->IsRunning()) {
				...
				// back to synchronous SetLength here â€” that would defeat
